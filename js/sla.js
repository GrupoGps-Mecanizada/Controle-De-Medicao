const SLAView = {
  ...
  slaCalc(r) {
    ...
    // Acumula SLA das etapas anteriores para estimar data de entrada na etapa atual
    const sIdx = STAGES.findIndex(st => st.id === r.stage);
    let offset = 0;
    for (let i = 0; i < sIdx; i++) offset += (STAGES[i].sla || 0);
    const entry = new Date(apDate);
    entry.setDate(entry.getDate() + offset);
